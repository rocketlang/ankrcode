...
export class OfflineAdapter {
    ...
    async loadModels() {
        ...
        try {
            ...
            console.warn('[OfflineAdapter] Failed to load models:', error.message);
        }
    }
    selectBestModel() {
        ...
        for (const preferred of RECOMMENDED_MODELS.coding) {
            const found = this.availableModels.find(m => m.name.toLowerCase().includes(preferred.toLowerCase()));
            if (found) {
    ...
    async completeOllama(systemPrompt, messages, model, options) {
    ...
    async completeOpenAICompatible(systemPrompt, messages, model, options) {
    ...
    async pullModel(model) {
        ...
        try {
            ...
            console.error('[OfflineAdapter] Pull failed:', error.message);
        }
        return false;
    }
    /**
...
// Singleton
let instance = null;
export function getOfflineAdapter() {
    if (!instance) {
...
//# sourceMappingURL=offline-adapter.js.map