 ...
 */
export async function analyzeScreenshot(_screenshot, // Reserved for future vision API
pageState, goal, previousSteps = []) {
...
Respond with valid JSON only (no markdown, no code blocks).`;
    const elementsDescription = pageState.elements
        ...
        if (el.text)
            desc += ` "${el.text.slice(0, 40)}"`;
        if (el.placeholder)
            desc += ` (placeholder: "${el.placeholder}")`;
        if (el.ariaLabel)
            desc += ` [${el.ariaLabel}]`;
        if (el.href)
            desc += ` -> ${el.href.slice(0, 40)}`;
        if (el.type)
            desc += ` (type: ${el.type})`;
        return desc;
    })
...
Respond with ONLY valid JSON (no markdown):
{
  ...
  "suggestedNextAction": {
    ...
    try {
        const response = await adapter.complete(systemPrompt, [{ role: 'user', content: userPrompt }], [], // No tools needed for vision analysis
        { model: 'claude' });
 ...
 */
function createFallbackAnalysis(goal, pageState) {
    const goalLower = goal.toLowerCase();
    let suggestedAction;
    // Heuristic: Check if we need to search
    if (goalLower.includes('search') || goalLower.includes('find')) {
        const searchInput = pageState.elements.find(el => el.type === 'search' || el.type === 'text' ||
 ...
 */
export async function isGoalCompleted(_screenshot, pageState, goal) {
    const adapter = getAIRouterAdapter();
...
Respond with JSON only: {"completed": true/false, "reason": "explanation"}`;
    try {
        const response = await adapter.complete('You are evaluating if a browser automation goal has been completed.', [{ role: 'user', content: prompt }], [], // No tools needed
        { model: 'claude' });
        const jsonMatch = response.content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
    ...
    if (goalLower.includes('click')) {
        ...
        if (clickMatch) {
            ...
            if (target.split(/\s+/).some(word => word.length > 3 && (urlLower.includes(word) || titleLower.includes(word)))) {
    ...
    const matchingWords = goalWords.filter(word => urlLower.includes(word) || titleLower.includes(word));
 ...
 */
export async function extractTextFromScreenshot(_screenshot) {
