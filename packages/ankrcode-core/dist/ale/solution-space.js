...
export class SolutionSpaceExplorer {
     ...
     */
    async explore(strategy, context, generateCandidate, scoreCandidate) {
     ...
     */
    async greedyExplore(context, generateCandidate, scoreCandidate) {
     ...
     */
    async annealingExplore(context, generateCandidate, scoreCandidate) {
        ...
        this.state.temperature = Math.max(this.config.minTemperature, this.state.temperature * this.config.coolingRate);
        // Reheat if stuck
     ...
     */
    async hybridExplore(context, generateCandidate, scoreCandidate) {
     ...
     */
    async reconstructionExplore(context, generateCandidate, scoreCandidate) {
     ...
     */
    async beamExplore(context, generateCandidate, scoreCandidate) {
     ...
     */
    async evolutionaryExplore(context, generateCandidate, scoreCandidate) {
     ...
     */
    reheat() {
        this.state.temperature = Math.min(this.config.initialTemperature, this.state.temperature * this.config.reheatingFactor);
        this.state.stuckCount = 0;
    }
    /**
    ...
    setTemperature(temperature) {
        this.state.temperature = Math.max(this.config.minTemperature, Math.min(this.config.initialTemperature, temperature));
    }
    /**
     * Inject a solution as current best (for warm starting)
     */
    warmStart(solution, score) {
...
export async function exploreSolutionSpace(strategy, context, generateCandidate, scoreCandidate) {
    return solutionSpaceExplorer.explore(strategy, context, generateCandidate, scoreCandidate);
}
//# sourceMappingURL=solution-space.js.map