 ...
 */
import { detectANKRPackages, checkAllServices, } from '../adapters/unified.js';
// ============================================================================
// ANSI Colors
// ============================================================================
const colors = {
...
function drawBox(title, content) {
    const maxWidth = Math.max(title.length + 4, ...content.map(line => stripAnsi(line).length + 4));
...
export async function runDiagnostics(config = {}) {
    ...
    const serviceInfo = services.map((s) => ({
        ...
        latency: s.latency,
        status: (!s.available ? 'error' : (s.latency && s.latency > 500 ? 'slow' : 'ok')),
    }));
    // Determine mode
...
export async function printDiagnostics(config = {}) {
...
export async function quickCheck() {
    ...
    const llmAvailable = result.packages.find(p => p.name === '@ankr/ai-router')?.available ||
        result.services.find(s => s.name === 'AI Proxy')?.available ||
        !!process.env.ANTHROPIC_API_KEY;
    const memoryAvailable = !!(result.packages.find(p => p.name === '@ankr/eon')?.available ||
        result.services.find(s => s.name === 'EON Memory')?.available);
    const toolsAvailable = !!(result.packages.find(p => p.name === '@ankr/mcp-tools')?.available ||
