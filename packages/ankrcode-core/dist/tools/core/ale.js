 ...
 */
import { optimize, quickOptimize, stopOptimization, getOptimizationSession, listOptimizations, buildWorkingMemoryContext, summarizeInsights, } from '../../ale/index.js';
/**
 * ALE Optimize Tool - Run multi-trial optimization
 */
export const aleOptimizeTool = {
    name: 'ALEOptimize',
...
- "Find the best algorithm for this sorting problem"`,
    ...
    async handler(params) {
        const { task, objective, constraints = [], strategy = 'hybrid', max_trials = 50, target_score = 0.95, timeout_seconds = 300, virtual_power_weight = 0.3, } = params;
        try {
            ...
            return {
                success: false,
                error: `ALE optimization failed: ${error.message}`,
            };
        }
    },
};
...
Use for simpler optimization tasks where you want quick results.`,
    ...
    async handler(params) {
        const { task, objective, preset = 'balanced' } = params;
        try {
            ...
            return {
                success: false,
                error: `Quick optimization failed: ${error.message}`,
            };
        }
    },
};
...
export const aleStatusTool = {
    ...
    async handler(params) {
        const { action, session_id } = params;
        try {
            switch (action) {
                case 'list': {
                    ...
                    const output = [
                        ...
                        '|----|--------|------|------------|--------|',
                        ...sessions.map(s => `| ${s.id.slice(0, 8)}... | ${s.status} | ${s.config.task.slice(0, 30)}... | ${s.bestScore?.totalScore.toFixed(3) || 'N/A'} | ${s.trials.length} |`),
                    ].join('\n');
            ...
            return {
                success: false,
                error: `ALE status error: ${error.message}`,
            };
        }
    },
};
