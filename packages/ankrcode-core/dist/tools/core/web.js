...
const braveProvider = {
    ...
    async search(query, options) {
        ...
        if (options.allowedDomains?.length) {
            results = results.filter(r => options.allowedDomains.some(d => r.domain.includes(d)));
        }
        if (options.blockedDomains?.length) {
            results = results.filter(r => !options.blockedDomains.some(d => r.domain.includes(d)));
        }
        return results;
    },
};
...
- Auto-selects best available search provider:
  ...
  - SearXNG (SEARXNG_URL) - Self-hosted option`,
    ...
    async handler(params) {
        const { query, allowed_domains, blocked_domains, max_results = 10 } = params;
        const options = {
        ...
        try {
            ...
            for (const fallbackProvider of searchProviders) {
                ...
                try {
                    ...
                    searchCache.set(cacheKey, { results, timestamp: Date.now() });
                    return formatSearchResults(results, query, `${fallbackProvider.name} (fallback)`, false);
                }
                catch {
                    continue;
                }
 ...
 */
function formatSearchResults(results, query, provider, fromCache) {
    if (results.length === 0) {
        return {
            success: true,
