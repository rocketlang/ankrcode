...
const AGENT_CONFIGS: Record<AgentType, AgentTypeConfig> = {
  researcher: {
    name: 'Researcher',
...
class AgentManager extends EventEmitter {
   ...
   * Spawn a new agent
   */
  async spawn(config: AgentConfig): Promise<AgentState> {
   ...
   * Update agent state
   */
  private updateState(id: string, updates: Partial<AgentState>): void {
    const state = this.agents.get(id);
    if (state) {
   ...
   * Log to agent
   */
  private log(id: string, level: AgentLogEntry['level'], message: string, data?: unknown): void {
    const state = this.agents.get(id);
    if (state) {
      state.logs.push({
        timestamp: new Date(),
   ...
   * List all agents
   */
  list(filter?: { status?: AgentState['status']; type?: AgentType }): AgentState[] {
   ...
   * Get agent logs
   */
  getLogs(id: string, limit = 50): AgentLogEntry[] {
    const state = this.agents.get(id);
    if (!state) return [];
...
export function spawnAgent(config: AgentConfig): Promise<AgentState> {
...
export function getAgentTypes(): Record<AgentType, AgentTypeConfig> {
