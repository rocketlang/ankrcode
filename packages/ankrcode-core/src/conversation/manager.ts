...
import { getAIRouterAdapter, type ToolDefinitionForLLM } from '../ai/router-adapter.js';
import { registry } from '../tools/registry.js';
import { getEONAdapter, remember, recall } from '../memory/eon-adapter.js';
...
export class ConversationManager {
  ...
  private aiAdapter = getAIRouterAdapter();
  private sessionId: string;
  private createdAt: Date;
  private eonAdapter = getEONAdapter();
   ...
   */
  private async callLLM(systemPrompt: string): Promise<LLMResponse> {
    ...
    try {
      const response = await this.aiAdapter.complete(
        systemPrompt,
        messages,
        tools,
        {
      ...
      throw new Error(`LLM error: ${(error as Error).message}`);
    }
  }
   ...
   */
  private mapModelToProvider(model: string): string | undefined {
   ...
   */
  private async callLocalModel(systemPrompt: string): Promise<LLMResponse> {
    try {
    ...
    for (const call of toolCalls) {
      try {
        ...
        if (call.name === 'Read' && result.success) {
          this.state.filesRead.add(call.parameters.file_path as string);
        }
      } catch (error) {
        results.push({
          success: false,
...
Working directory: ${process.cwd()}`;
  ...
  private getSwayamPrompt(language: SupportedLanguage): string {
...
Working directory: ${process.cwd()}`,
   ...
   */
  private getToolDefinitions(): ToolDefinitionForLLM[] {
    return registry.getAnthropicFormat() as ToolDefinitionForLLM[];
  }
   ...
   */
  async loadConversation(sessionId: string): Promise<boolean> {
    if (!this.config.persistenceEnabled) {
      return false;
    }
    ...
    try {
      ...
      const saved: SavedConversation = JSON.parse(result.memory.content);
  ...
  async listConversations(): Promise<Array<{
    ...
    try {
      ...
      return results.map((r: any) => {
        try {
          const saved: SavedConversation = JSON.parse(r.memory.content);
          return {
   ...
   */
  exportToJSON(): string {
    const exported: SavedConversation = {
      sessionId: this.sessionId,
      config: {
        model: this.config.model,
        provider: this.config.provider,
  ...
  importFromJSON(json: string): boolean {
    try {
      const saved: SavedConversation = JSON.parse(json);
