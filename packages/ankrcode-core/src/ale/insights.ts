...
export interface InsightConfig {
  // Thresholds
  successThreshold: number;      // Score above this = success
  failureThreshold: number;      // Score below this = failure
  improvementThreshold: number;  // Delta above this = notable improvement
...
export class InsightsGenerator {
   ...
   */
  private generateOutcomeInsight(trial: Trial, allTrials: Trial[]): Insight | null {
    ...
    const approach = trial.solution.metadata.approach as string || 'unknown';
   ...
   */
  private generateDeltaInsight(trial: Trial, allTrials: Trial[]): Insight | null {
    const prevTrial = allTrials[allTrials.length - 2];
    if (!prevTrial) return null;
    ...
    const approach = trial.solution.metadata.approach as string || 'unknown';
    const prevApproach = prevTrial.solution.metadata.approach as string || 'unknown';
   ...
   */
  private generateStrategyInsight(trial: Trial, allTrials: Trial[]): Insight | null {
    const approach = trial.solution.metadata.approach as string;
    if (!approach) return null;

    // Find all trials with same approach
    const sameApproach = allTrials.filter(
      t => t.solution.metadata.approach === approach
    );

    if (sameApproach.length < 2) return null;
   ...
   */
  private generatePatternInsights(trial: Trial, allTrials: Trial[]): Insight[] {
    const insights: Insight[] = [];
   ...
   */
  private detectPatterns(trials: Trial[]): DetectedPattern[] {
   ...
   */
  private generateComponentInsights(trial: Trial): Insight[] {
    const insights: Insight[] = [];
  ...
  private extractTags(trial: Trial): string[] {
    const tags: string[] = [];
  ...
  private isPlateaued(scores: number[]): boolean {
  ...
  private isOscillating(scores: number[]): boolean {
    if (scores.length < 4) return false;
  ...
  private findCommonApproach(trials: Trial[]): string | null {
