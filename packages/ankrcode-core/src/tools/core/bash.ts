...
- block=true waits for completion, block=false returns current status`,
  ...
  async handler(params): Promise<ToolResult> {
    const { task_id, block = true, timeout = 30000 } = params as {
      task_id: string;
      block?: boolean;
      timeout?: number;
    };
...
function runCommand(command: string, timeout: number): Promise<ToolResult> {
  return new Promise((resolve) => {
...
function runInBackground(command: string): ToolResult {
...
function checkCommandSecurity(command: string): SecurityCheckResult {
  const dangerous = [
...
export function getBackgroundTasks(): Map<string, {
  process: ChildProcess;
  output: string;
  status: 'running' | 'completed' | 'failed';
}> {
