 ...
 */
function createBuiltinFn(name, params) {
    return {
        type: 'function',
        name,
...
export class ModuleLoader {
    ...
    parser;
    executor;
    loading = new Set(); // Circular dependency detection
    constructor(config = {}) {
     ...
     */
    async load(importPath, fromFile) {
    ...
    async loadFile(resolution) {
        ...
        for (const cmd of parseResult.commands) {
            if (cmd.tool === 'import' || cmd.tool === 'use' || cmd.tool === 'उपयोग') {
                const dep = cmd.parameters.path || cmd.parameters.from;
                if (dep) {
     ...
     */
    async loadJavaScriptModule(resolution, _source) {
        try {
            ...
            throw new Error(`Failed to load JavaScript module ${resolution.path}: ${error.message}`);
        }
    }
    /**
     * Load a package module
     */
    async loadPackage(resolution) {
        ...
        try {
            ...
            throw new Error(`Failed to load package ${resolution.originalPath}: ${error.message}`);
        }
    }
    /**
     * Load a URL module
     */
    async loadUrl(resolution) {
        ...
        try {
            ...
            throw new Error(`Failed to fetch module from ${resolution.path}: ${error.message}`);
        }
    }
    /**
     * Import specific items from a module
     */
    async import(spec, fromFile) {
        const module = await this.load(spec.path, fromFile);
        const result = {};
        // Import as namespace
        if (spec.namespace) {
            result[spec.namespace] = module.exports;
            return result;
        }
        // Import all
