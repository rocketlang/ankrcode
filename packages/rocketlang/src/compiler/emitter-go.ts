...
export class GoEmitter {
  private config: GoEmitterConfig;
  ...
  private emitExpression(expr: Expression): string {
    switch (expr.type) {
      ...
      default:
        return `/* Unsupported: ${(expr as Expression).type} */`;
    }
  }
   ...
   */
  private mapType(type: RocketType): string {
    switch (type.kind) {
      case 'primitive':
        switch (type.name) {
          case 'number':
 ...
 * Create a Go emitter
 */
export function createGoEmitter(config?: Partial<GoEmitterConfig>): GoEmitter {
 ...
 * Emit program to Go
 */
export function emitGo(program: Program, config?: Partial<GoEmitterConfig>): GoEmitResult {
